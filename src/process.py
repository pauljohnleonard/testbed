...
class Processor:
   ...
   # feed moving average into the peak detector adjust time for processing latency
        peak=self.peak_client.process(val1,self.time)
...
class BPMFilter:
    ...
    def process(self,bpm,t,val):
        ...
        if bpm < MIN_BPM or bpm > MAX_BPM:
            print " BPM OUT OF BOUNDS ",bpm
            return
        
        
        if abs(bpm-self.median.median_val()) > MAX_DEVIATION_BPM:
            print " REFECTED: Deviation of BPM  (bpm,median)",bpm,self.median.median_val()
            return
    
        if abs(bpm-self.bpm_last) > MAX_DELTA_BPM:
            print " REFECTED: Change of BPM ",abs(bpm-self.bpm_last),bpm,self.bpm_last
            return
        
        self.average = self.average*self.fact1+self.fact2*bpm
        self.average_fast = self.average_fast*self.fact1f+self.fact2f*bpm

        self.median.process(bpm) 
           
        self.client.process(bpm,t,val)
        self.bpm_last=bpm
        self.BPMfilt.append((t,bpm))
...
class Peaker:
    ...
    def __init__(self,client,dt):
        ...
        self.thresh_delay=Delay(THRESH_DELAY)
    ...
    def process(self,f,t):
        ...
        #  adaptive thresh      
        thresh1=self.thresh1 = self.thresh_delay.process(self.averN.process(min(f,self.threshLimit)*THRESH_SCALE))   
